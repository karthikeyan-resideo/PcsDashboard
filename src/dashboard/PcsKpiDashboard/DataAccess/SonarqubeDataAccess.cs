...
namespace PcsKpiDashboard.DataAccess
{
    public class SonarqubeDataAccess : ISonarqubeDataAccess
    {
        ...
        public SonarqubeDataAccess(ApplicationDbContext context, IConfiguration configuration, ICustomLogger logger)
        {
            _context = context;
        ...
        public async Task<List<CodeCoverageDataModel>> GetCoverage()
        {
            ...
            _logger.ProcessLogs($"Project details {JsonSerializer.Serialize(projects)}");
        ...
        public async Task<List<SonarqubeCoverageDetail>> GetSonarqubeCoverageDetail()
        {
            ...
            {
                string projectName = projects.FirstOrDefault(x => x.Id == detailData.ProjectId).ProjectName;
